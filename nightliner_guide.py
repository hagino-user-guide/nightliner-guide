...
def main():
    ...
    seat4_overhead_photo = image_data_uri("seat_overhead_console_web.jpg")
    ...
    seat3_usb_photo = image_data_uri("seat3_usb_port_web.jpg")
    footrest_photo = image_data_uri("seat_footrest_web.jpg")
    amenity_blanket_photo = image_data_uri("blanket_web.jpg")
    amenity_water_photo = image_data_uri("water_web.jpg")
    amenity_cushion_photo = image_data_uri("cushion_web.jpg")
    amenity_mask_photo = image_data_uri("alface_mask_web.jpg")
    toilet_map_photo = image_data_uri("toilet_map.png")
    toilet_entrance_photo = image_data_uri("toilet_entrance_web.jpg")
    toilet_inside_photo = image_data_uri("toilet_inside_web.jpg")
    sa_time_board_photo = image_data_uri("sa_time_board_web.jpg")
    emergency_exit_map_3row = image_data_uri("emergency_exit_map_3row.png")
    emergency_exit_map_4row = image_data_uri("emergency_exit_map_4row.png")
    emergency_exit_overview = image_data_uri("emergency_exit_overview.jpg")
    emergency_exit_handle = image_data_uri("emergency_exit_handle.jpg")
    ...
    html = """
        ...
        <head>
            ...
            <style>
                ...
                #page-today .step-action-list .step-note { padding-left:0; margin-top:2px; color:#9A3412; font-size:10.8px; line-height:1.45; font-weight:700; }
                #page-today .step-action-list .step-note::before { display:none; }
                ...
                #page-help .emergency-exit-section { width:100%; min-width:0; }
                #page-help .emergency-exit-heading { display:flex; align-items:center; gap:7px; margin:0 0 9px; color:#1E293B; font-size:12.8px; line-height:1.4; font-weight:900; }
                #page-help .emergency-exit-heading i { width:24px; height:24px; display:inline-flex; align-items:center; justify-content:center; border-radius:8px; color:#005BAC; background:#EFF6FF; font-size:15px; }
                #page-help .emergency-exit-photo-heading { margin-top:15px; }
                #page-help .emergency-exit-tabs { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:6px; margin-bottom:9px; padding:4px; border-radius:11px; background:#F1F5F9; }
                #page-help .emergency-exit-tab { min-height:38px; padding:8px 9px; border:0; border-radius:8px; background:transparent; color:#64748B; font-size:11.8px; line-height:1.3; font-weight:900; cursor:pointer; }
                #page-help .emergency-exit-tab.is-active { background:#FFFFFF; color:#005BAC; box-shadow:0 2px 8px rgba(15,23,42,.08); }
                #page-help .emergency-exit-map-wrap { position:relative; width:100%; height:238px; overflow:hidden; border:1px solid #E2E8F0; border-radius:12px; background:#FFFFFF; }
                #page-help .emergency-exit-map { display:none; position:absolute; left:50%; top:50%; width:176%; max-width:none; height:auto; padding:0; transform:translate(-50%,-50%); box-sizing:border-box; }
                #page-help .emergency-exit-map.is-active { display:block; }
                #page-help .emergency-exit-photo-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:8px; }
                #page-help .emergency-exit-photo-card { min-width:0; margin:0; overflow:hidden; border:1px solid #E2E8F0; border-radius:12px; background:#FFFFFF; }
                #page-help .emergency-exit-photo-card img { display:block; width:100%; height:138px; object-fit:cover; object-position:center; background:#F8FAFC; }
                #page-help .emergency-exit-photo-card figcaption { padding:8px 7px; border-top:1px solid #EEF2F7; color:#475569; font-size:10.8px; line-height:1.35; font-weight:850; text-align:center; }
                #page-help .emergency-exit-guide-list { margin:13px 0 0; padding:0; list-style:none; display:flex; flex-direction:column; gap:8px; }
                #page-help .emergency-exit-guide-list li { position:relative; padding-left:17px; color:#475569; font-size:11.8px; line-height:1.65; font-weight:650; }
                #page-help .emergency-exit-guide-list li::before { content:''; position:absolute; left:2px; top:.72em; width:6px; height:6px; border-radius:50%; background:#005BAC; }
                #page-help .emergency-exit-guide-list .emergency-exit-caution { margin-top:2px; padding:10px 11px 10px 29px; border:1px solid #FECACA; border-left:4px solid #EF4444; border-radius:10px; background:#FFF5F5; color:#991B1B; font-weight:800; }
                #page-help .emergency-exit-guide-list .emergency-exit-caution::before { left:13px; top:50%; transform:translateY(-50%); background:#EF4444; }
                @media (max-width:380px) {
                    #page-help .emergency-exit-photo-card img { height:122px; }
                    #page-help .emergency-exit-map-wrap { height:218px; }
                    #page-help .emergency-exit-map { width:182%; }
                }
                ...
                .luggage-limit-row { display: grid; grid-template-columns: 1fr; gap: 8px; margin-top: 10px; }
                ...
                .amenity-card-photo { width:100%; height:78px; display:block; object-fit:cover; object-position:center; border-radius:11px; border:1px solid #D8E3F0; background:#F8FAFC; }
                .amenity-modal-photo-wrap { width:100%; display:flex; justify-content:center; align-items:center; padding:2px 0 4px; box-sizing:border-box; }
                .amenity-modal-photo { display:block; width:100%; max-width:340px; max-height:68vh; object-fit:contain; border-radius:14px; background:#F8FAFC; }
                .amenity-select-card { justify-content:flex-start; padding:8px 8px 10px; gap:8px; min-height:116px; }
                .amenity-detail-layout { display:grid; grid-template-columns:112px minmax(0,1fr); gap:12px; align-items:start; }
                .amenity-detail-photo { width:112px; height:88px; object-fit:cover; object-position:center; border-radius:11px; border:1px solid #D8E3F0; background:#F8FAFC; }
                .amenity-detail-copy { min-width:0; }

                /* 車内トイレ：見出しを省いたシンプルな案内UI */
                #page-manner .rest-guide-copy {
                    margin:12px 0 0;
                    padding:2px 0;
                }
                #page-manner .rest-guide-heading {
                    margin:0 0 3px;
                    color:#17324F;
                    font-size:12.4px;
                    line-height:1.4;
                ...
                #page-manner .rest-guide-item + .rest-guide-item {
                    border-top:1px solid #DCEDEA;
                }
                #page-manner .rest-guide-icon {
                    width:22px;
                    height:22px;
                    display:flex;
                    align-items:center;
                ...
                #page-manner .rest-guide-icon svg {
                    width:18px;
                    height:18px;
                    fill:none;
                    stroke:currentColor;
                ...
                #page-manner .rest-visual-flow { display:grid; grid-template-columns:1fr; gap:10px; margin-top:12px; }
                #page-manner .rest-visual-card {
                    overflow:hidden;
                ...
                #page-manner .rest-visual-card img { width:100%; height:150px; display:block; object-fit:cover; object-position:center; background:#F8FAFC; }
                #page-manner .rest-visual-card.is-map img { height:auto; max-height:154px; object-fit:contain; padding:8px; box-sizing:border-box; background:#F7FBFA; }
                #page-manner .rest-visual-caption {
                    display:grid;
                ...
                #page-manner .rest-visual-caption-icon {
                    width:28px;
                    height:28px;
                    display:flex;
                    align-items:center;
                ...
                #page-manner .rest-visual-caption-copy { min-width:0; }
                #page-manner .rest-visual-title { display:block; margin:0 0 3px; color:#17324F; font-size:12px; line-height:1.35; font-weight:900; }
                #page-manner .rest-visual-text { display:block; margin:0; color:#64748B; font-size:11.3px; line-height:1.55; font-weight:600; }
                #page-manner .sa-board-card { margin-top:12px; overflow:hidden; border:1px solid #F4D28A; border-radius:13px; background:#FFFDF7; }
                #page-manner .sa-board-card img { width:100%; height:184px; display:block; object-fit:cover; object-position:center 46%; }
                #page-manner .sa-board-caption { display:flex; align-items:flex-start; gap:9px; padding:10px 11px; color:#6B4708; font-size:11.7px; line-height:1.55; font-weight:800; }
                #page-manner .sa-board-caption i { margin-top:1px; color:#D97706; font-size:17px; flex:0 0 auto; }
                #page-manner .rest-important-copy { margin-top:10px; padding:10px 11px; border-left:4px solid #F59E0B; border-radius:10px; background:#FFF8E8; color:#6B4708; font-size:11.7px; line-height:1.68; font-weight:750; }
                @media (max-width:380px) {
                    .amenity-card-photo { height:68px; }
                    .amenity-select-card { min-height:104px; }
                    .amenity-detail-layout { grid-template-columns:82px minmax(0,1fr); gap:9px; }
                    .amenity-detail-photo { width:82px; height:68px; }
                    #page-manner .rest-visual-card img { height:132px; }
                    #page-manner .sa-board-card img { height:164px; }
                }
                ...
                .schedule-announcement-card { margin-top:10px; background:#F8FAFC; border:1px solid #D8E3F0; border-radius:12px; padding:11px 12px; }
                .schedule-announcement-title { display:flex; align-items:center; gap:6px; color:#005bac; font-size:12.5px; font-weight:900; margin-bottom:0; }
                .schedule-announcement-text { color:#334155; font-size:12px; line-height:1.65; font-weight:600; white-space:pre-wrap; }
                .schedule-announcement-header { display:flex; align-items:center; justify-content:space-between; cursor:pointer; user-select:none; -webkit-tap-highlight-color:transparent; }
                .schedule-announcement-arrow { font-size:12px; color:#94A3B8; transition:transform 0.2s; margin-left:8px; flex:0 0 auto; }
                .schedule-announcement-card.open .schedule-announcement-arrow { transform:rotate(90deg); }
                .schedule-announcement-content { display:grid; grid-template-rows:0fr; opacity:0; overflow:hidden; transition:grid-template-rows .24s ease, opacity .2s ease; }
                .schedule-announcement-card.open .schedule-announcement-content { grid-template-rows:1fr; opacity:1; margin-top:9px; }
                .schedule-announcement-inner { min-height:0; overflow:hidden; }
                ...
                .station-menu-group-title.group-tohoku { color: #7C3AED; background: #F5F3FF; border: 1px solid #DDD6FE; }
                .station-guide-card { background:#FFFFFF; border:1px solid #E2E8F0; border-radius:13px; padding:14px; margin:0; color:#334155; font-size:13px; line-height:1.85; text-align:left; overflow-wrap:anywhere; }
                .station-guide-label { color:#005bac; font-size:12.5px; font-weight:900; margin:0 0 5px; }
                .station-guide-name { color:#0F172A; font-size:13.5px; font-weight:800; line-height:1.65; margin:0 0 10px; }
                .station-guide-text { margin:0; white-space:normal; }
                .station-guide-text + .station-guide-text { margin-top:10px; }
                .station-guide-important { margin-top:12px; padding:11px 12px; background:#FFF7ED; border:1px solid #FED7AA; border-radius:10px; color:#9A3412; }
                .station-guide-important strong { display:block; margin-bottom:5px; color:#C2410C; }
                .station-guide-link { display:block; margin-top:9px; color:#005bac; font-weight:800; text-decoration:underline; text-underline-offset:2px; word-break:break-all; }
                ...
                .hotspot-btn { position:absolute; width:44px; min-width:44px; height:44px; box-sizing:border-box; border:none; background:transparent; color:#FFFFFF; border-radius:50%; cursor:pointer; z-index:30; display:flex; align-items:center; justify-content:center; padding:0; font-size:13px; font-weight:900; line-height:1; transition:top .32s cubic-bezier(.25,.8,.25,1),left .32s cubic-bezier(.25,.8,.25,1),transform .18s ease,filter .18s ease; -webkit-tap-highlight-color:transparent; touch-action:manipulation; }
                .hotspot-btn::before { content:''; position:absolute; left:50%; top:50%; width:32px; height:32px; border:1px solid rgba(255,255,255,.74); background:radial-gradient(circle,rgba(255,255,255,.36) 0%,rgba(93,182,255,.18) 48%,rgba(0,91,172,.07) 100%); border-radius:50%; transform:translate(-50%,-50%) scale(.74); animation:seatPinPulse 2.35s infinite; pointer-events:none; z-index:0; box-shadow:0 0 0 1px rgba(0,91,172,.16),0 0 18px rgba(93,182,255,.34); }
                .hotspot-btn::after { content:''; position:absolute; left:50%; top:50%; width:16px; height:16px; border-radius:50%; border:2px solid rgba(255,255,255,.98); background:radial-gradient(circle at 34% 28%,#FFFFFF 0%,#9FE2FF 22%,#1C91DD 58%,#005BAC 100%); box-shadow:0 0 0 4px rgba(255,255,255,.2),0 0 16px rgba(93,182,255,.52),0 6px 14px rgba(0,48,106,.3),inset 0 1px 0 rgba(255,255,255,.72); transform:translate(-50%,-50%); pointer-events:none; z-index:1; }
                .hotspot-btn:hover { transform:translateY(-1px) scale(1.03); }
                .hotspot-btn:active { transform:scale(.94); }
                .hotspot-btn i { display:none !important; }
                .hotspot-btn.active { background:transparent !important; border-color:transparent !important; box-shadow:none !important; transform:translateY(-1px) scale(1.08); filter:saturate(1.08); }
                .hotspot-btn.active::before { width:38px; height:38px; border:1.5px solid rgba(255,255,255,.88); background:radial-gradient(circle,rgba(255,255,255,.34) 0%,rgba(184,148,70,.2) 52%,rgba(0,91,172,.08) 100%); animation:none; transform:translate(-50%,-50%) scale(1); box-shadow:0 0 0 3px rgba(184,148,70,.16),0 0 22px rgba(184,148,70,.32); }
                .hotspot-btn.active::after { width:18px; height:18px; background:radial-gradient(circle at 34% 28%,#FFFFFF 0%,#FFEAA8 25%,#F2C95B 56%,#005BAC 100%); border:2px solid #FFFFFF; box-shadow:0 0 0 5px rgba(184,148,70,.18),0 0 22px rgba(184,148,70,.38),0 7px 16px rgba(0,49,109,.28),inset 0 1px 0 rgba(255,255,255,.75); }
                ...
                @media (max-width:380px){ .hotspot-btn::before{width:30px;height:30px}.hotspot-btn::after{width:15px;height:15px}.hotspot-btn.active::before{width:36px;height:36px}.hotspot-btn.active::after{width:17px;height:17px}.hotspot-btn i{display:none!important}.interactive-tooltip{width:210px}.seat-hotspot-info{padding:12px 13px 13px}.seat-air-photo{height:96px} }
                @media (max-width:355px){ .hotspot-btn::before{width:29px;height:29px}.hotspot-btn::after{width:14px;height:14px}.hotspot-btn.active::before{width:34px;height:34px}.hotspot-btn.active::after{width:16px;height:16px}.hotspot-btn i{display:none!important}.interactive-tooltip{width:202px;padding:12px 12px 11px}.seat-hotspot-info-title{font-size:13.5px}.seat-hotspot-info-desc{font-size:11.8px}.seat-air-card{padding:7px}.seat-air-photo{height:88px}.seat-air-card strong{font-size:11.8px}.seat-air-card span{font-size:11.1px}.seat-equipment-photo{height:128px}.seat-equipment-step-photo{height:132px}.seat-equipment-photo-contain{height:190px} }
                ...
                #page-manner .manner-wifi { --card-accent:#3E82D8; --card-dark:#2F6CB8; --card-soft:#FFFFFF; --card-tint:#FFFFFF; --card-line:#E6EAF0; --card-icon-bg:#EFF6FF; --card-icon-border:#D7E7FA; }
                /* アメニティ一覧：アイコン＋名称のみ。詳細はモーダルで表示 */
                #page-manner .manner-amenity .amenity-select-card {
                    min-height: 112px;
                ...
                #page-manner .manner-amenity .amenity-line-icon-wrap {
                    width: 54px;
                    height: 54px;
                    border-radius: 15px;
                    background: #FFF4E2;
                ...
                #page-manner .manner-amenity .amenity-line-icon {
                    width: 31px;
                    height: 31px;
                }
                #page-manner .manner-amenity .amenity-line-icon * {
                ...
                #page-manner .manner-amenity .amenity-card-name {
                    font-size: 12px;
                    line-height: 1.35;
                    color: #142B45;
                    font-weight: 900;
                }
                .amenity-modal-content { width:100%; box-sizing:border-box; }
                .amenity-modal-content .amenity-modal-photo-wrap { padding:0 0 13px; }
                .amenity-modal-content .amenity-modal-photo {
                    width:100%;
                    max-width:360px;
                    max-height:55vh;
                    object-fit:contain;
                ...
                .amenity-modal-description .amenity-detail-highlight {
                    margin-top:10px;
                }
                ...
                <div class="app-scroll-body" id="js-scroll-body">
                            ...
                            <div class="luggage-size-card">
                                <div class="luggage-size-visual">
                                    ...
                                    <svg class="luggage-size-svg" viewBox="35 15 245 205" xmlns="http://www.w3.org/2000/svg" aria-label="トランク預け荷物のサイズ目安">
                                        ...
                                        <text x="79" y="175" text-anchor="middle" font-size="11" font-weight="900" fill="#005BAC">B</text>

                                        <!-- 奥行 -->
                                        <rect x="148" y="163" width="68" height="18" rx="9" fill="#EFF6FF" stroke="#BFDBFE" stroke-width="1"/>
                                        <text x="182" y="175" text-anchor="middle" font-size="11" font-weight="900" fill="#005BAC">C</text>

                                        <!-- 高さ -->
                                        <rect x="198" y="91" width="74" height="20" rx="10" fill="#EFF6FF" stroke="#BFDBFE" stroke-width="1"/>
                                        <text x="235" y="104" text-anchor="middle" font-size="11" font-weight="900" fill="#005BAC">A</text>
                                    </svg>
                                </div>
                                <p class="luggage-main-limit">トランク預けの目安<strong>A＋B＋C<br>3辺合計170cm以内</strong></p>
                                <div class="luggage-limit-row">
                                    <div class="luggage-limit-chip"><strong>お1人様 1つ程度</strong></div>
                                </div>
                            </div>
                        ...
                        <div class="info-card today-notice-card">
                            ...
                            <div class="today-priority-row">
                                ...
                                <span>
                                    ...
                                    <strong class="today-priority-main">出発15分前です</strong>
                                </span>
                            </div>
                        ...
                        <div class="step-outer-container-js">
                            <div class="step-scroll-wrapper" id="js-step-scroller">
                                <div class="step-premium-card">
                                    ...
                                    <div class="step-card-title">出発15分前までに集合</div>
                                ...
                                <div class="step-premium-card">
                                    ...
                                    <div class="step-action-list"><span>お名前と予約番号を伝える</span><span>予約内容を確認</span></div>
                                </div>
                                <div class="step-premium-card">
                                    ...
                                    <div class="step-action-list"><span>トランク荷物を渡す</span><span>手荷物タグを保管</span><span class="step-note">※仙台便では手荷物タグはございません。</span></div>
                                </div>
                                <div class="step-premium-card">
                                    ...
                                    <div class="step-action-list"><span>4列シート：当日ご案内された座席へ着席</span><span>3列シート：座席指定済みは指定席へ、未指定の場合は当日ご案内された座席へ着席</span></div>
                                </div>
                                <div class="step-premium-card">
                                    ...
                                    <div class="step-action-list"><span>シートベルトを着用</span></div>
                                </div>
                            </div>
                        </div>
                        
                        <div class="heading">運行スケジュール</div>
                        <p style="font-size:12px; color:#64748B; margin-bottom:10px;">ご利用便を選択すると、運行スケジュールをご確認いただけます。\n※運行スケジュールに記載の各時刻は目安です。道路状況や天候などにより前後する場合がありますので、あらかじめご了承ください。</p>
                        <select class="bus-select-box" id="bus_select">
                            <option value="bus_nt1001">ナイトライナー NT1001便（東京⇒大阪・京都）4列スタンダード</option>
                            <option value="bus_nt002">ナイトライナー NT002便（大阪・京都⇒高尾・新宿・東京）4列スタンダード</option>
                            <option value="bus_nt1003">ナイトライナー NT1003便（東京⇒奈良・大阪・京都）3列独立（トイレ有）</option>
                            <option value="bus_nt004">ナイトライナー NT004便（大阪・奈良・京都⇒横浜・新宿・東京）3列独立（トイレ有）</option>
                            <option value="bus_nt1005">ナイトライナー NT1005便（大宮・新宿・八王子⇒京都・大阪・天王寺）3列独立（トイレ有）</option>
                            <option value="bus_nt006">ナイトライナー NT006便（天王寺・大阪・京都⇒高尾・新宿・大宮）3列独立（トイレ有）</option>
                            <option value="bus_nt011">ナイトライナー NT011便（新宿⇒仙台）4列スタンダード</option>
                            <option value="bus_nt012">ナイトライナー NT012便（仙台⇒新宿）4列スタンダード</option>
                            <option value="bus_nt013">ナイトライナー NT013便（東京⇒仙台）4列スタンダード</option>
                            <option value="bus_nt014">ナイトライナー NT014便（仙台⇒東京）4列スタンダード</option>
                        </select>
                    ...
                    <div id="page-recline" class="page-view">
                        ...
                        <div class="seat-map-wrapper view-seat3" id="seat-canvas-container">
                            ...
                            <div class="hotspot-btn" id="pin-table-legrest" data-key="tableLegrest" aria-label="簡易テーブル・レッグレストバー"><i class="ph-bold ph-sliders-horizontal"></i></div>
...
<div id="page-manner" class="page-view">
                        ...
                        <div class="manner-accordion-list">
                            ...
                            <div class="simple-accordion manner-main-accordion manner-night">
                                ...
                                <div class="accordion-content"><div class="manner-accordion-inner">
                                    <div class="manner-subitem-list">
                                        ...
                                        <div class="manner-subitem"><span class="manner-subitem-icon"><i class="ph-bold ph-clock"></i></span><div><p class="manner-subitem-title">消灯時間</p><p class="manner-subitem-text">最終経由地を出発してから約10分後に消灯します。消灯前にアナウンスを行います。</p></div></div>
                                ...
                                <div class="accordion-content"><div class="manner-accordion-inner">
                                    <div class="amenity-section-shell">
                                        <div class="amenity-grid" id="js-amenity-grid">
                                            <button type="button" class="amenity-select-card" data-amenity="blanket" aria-label="ブランケットの写真と説明を表示">
                                                <span class="amenity-line-icon-wrap"><svg class="amenity-line-icon" viewBox="0 0 32 32" aria-hidden="true"><path d="M7 11.5h18a2 2 0 0 1 2 2v10H5v-10a2 2 0 0 1 2-2Z"/><path d="M8.5 11.5V9.8A1.8 1.8 0 0 1 10.3 8h11.4a1.8 1.8 0 0 1 1.8 1.8v1.7"/><path d="M8.5 15.5h15v8h-15Z"/><path d="M8.5 18h15"/></svg></span>
                                                <span class="amenity-card-name">ブランケット</span>
                                            </button>
                                            <button type="button" class="amenity-select-card" data-amenity="water" aria-label="ミネラルウォーターの写真と説明を表示">
                                                <span class="amenity-line-icon-wrap"><svg class="amenity-line-icon" viewBox="0 0 32 32" aria-hidden="true"><path d="M13 5h6"/><path d="M13.7 5v4.2l-2 2.3v13.2A2.3 2.3 0 0 0 14 27h4a2.3 2.3 0 0 0 2.3-2.3V11.5l-2-2.3V5"/><path d="M11.7 16h8.6"/><path d="M14 20.5c1.1-1.9 2.9-1.9 4 0"/></svg></span>
                                                <span class="amenity-card-name">ミネラルウォーター</span>
                                            </button>
                                            <button type="button" class="amenity-select-card" data-amenity="cushion" aria-label="腰あてクッションの写真と説明を表示">
                                                <span class="amenity-line-icon-wrap"><svg class="amenity-line-icon" viewBox="0 0 32 32" aria-hidden="true"><rect x="8" y="6" width="16" height="20" rx="4"/><path d="M10.5 13c3.1 2.1 7.9 2.1 11 0"/><path d="M10.5 19c3.1-2.1 7.9-2.1 11 0"/></svg></span>
                                                <span class="amenity-card-name">腰あてクッション</span>
                                            </button>
                                            <button type="button" class="amenity-select-card" data-amenity="mask" aria-label="フェイスマスクの写真と説明を表示">
                                                <span class="amenity-line-icon-wrap"><svg class="amenity-line-icon" viewBox="0 0 32 32" aria-hidden="true"><path d="M10 9.5c1.8-1.8 3.8-2.7 6-2.7s4.2.9 6 2.7v9.2c0 4-2.6 7.3-6 7.3s-6-3.3-6-7.3Z"/><path d="M10 12.5 6.8 11v6.5L10 19"/><path d="m22 12.5 3.2-1.5v6.5L22 19"/><path d="M13 16h.1M19 16h.1"/><path d="M13.4 20.2c1.6 1.1 3.6 1.1 5.2 0"/></svg></span>
                                                <span class="amenity-card-name">フェイスマスク</span>
                                            </button>
                                        </div>
                                    </div>
                                </div></div>
                            ...
                            <div class="simple-accordion manner-main-accordion manner-wifi">
                                <div class="accordion-header" role="button" tabindex="0" aria-expanded="false">
                                    <span class="manner-main-icon" aria-hidden="true"><i class="ph-bold ph-wifi-high"></i></span>
                                    <span class="manner-title-stack"><span class="manner-main-title">車内Wi-Fi</span></span>
                                    <span class="accordion-arrow">▶</span>
                                </div>
                                <div class="accordion-content"><div class="manner-accordion-inner">
                                    <div class="manner-subitem-list">
                                        <div class="manner-subitem is-key">
                                            <span class="manner-subitem-icon"><i class="ph-bold ph-wifi-high"></i></span>
                                            <div>
                                                <p class="manner-subitem-title">車内Wi-Fiについて</p>
                                                <p class="manner-subitem-text">ご乗車のバスでは、車内Wi-Fiをご利用いただけます。</p>
                                                <p class="manner-subitem-text">※ご利用には端末の登録が必要となります。</p>
                                            </div>
                                        </div>
                                        <div class="manner-subitem">
                                            <span class="manner-subitem-icon"><i class="ph-bold ph-gear-six"></i></span>
                                            <div>
                                                <p class="manner-subitem-title">設定方法</p>
                                                <ol class="manner-mini-list" style="list-style:none; counter-reset:wifi-step;">
                                                    <li style="padding-left:0;">① お使いの端末のWi-Fi設定画面を開いてください。</li>
                                                    <li style="padding-left:0;">② Wi-Fi一覧から、以下のSSIDを選択してください。</li>
                                                </ol>
                                                <div style="margin:8px 0; padding:9px 10px; border:1px solid #D7E7FA; border-radius:10px; background:#F8FBFF;">
                                                    <span style="display:block; color:#64748B; font-size:10.8px; font-weight:800; margin-bottom:3px;">SSID</span>
                                                    <span style="display:block; color:#17324F; font-size:12.5px; line-height:1.4; font-weight:900; overflow-wrap:anywhere;">Wi-Fi_by_anobus.jp</span>
                                                </div>
                                                <p class="manner-subitem-text">③ 利用規約に同意し、生年月日を登録してください。</p>
                                            </div>
                                        </div>
                                    </div>
                                </div></div>
                            ...
                            <div class="simple-accordion manner-main-accordion manner-rest">
                                ...
                                <div class="accordion-content"><div class="manner-accordion-inner">
                                    <div class="manner-subitem-list">
                                        <div class="manner-subitem"><span class="manner-subitem-icon"><i class="ph-bold ph-toilet"></i></span><div><p class="manner-subitem-title">車内トイレ</p><div class="rest-toilet-compare"><div class="rest-seat-status status-yes"><span class="rest-seat-icon"><svg class="rest-seat-svg" viewBox="0 0 24 24" aria-hidden="true"><rect x="7" y="4" width="10" height="11" rx="2"/><path d="M6 15h12v3H6z"/><path d="M8 21v-3M16 21v-3"/><path d="M5 8v6M19 8v6"/></svg></span><span><span class="rest-seat-name">3列独立シート</span><span class="rest-seat-badge">✓ 車内トイレあり</span></span></div><div class="rest-seat-status status-no"><span class="rest-seat-icon"><svg class="rest-seat-svg" viewBox="0 0 24 24" aria-hidden="true"><rect x="7" y="4" width="10" height="11" rx="2"/><path d="M6 15h12v3H6z"/><path d="M8 21v-3M16 21v-3"/><path d="M5 8v6M19 8v6"/></svg></span><span><span class="rest-seat-name">4列シート（スタンダード）</span><span class="rest-seat-badge">× 車内トイレなし</span></span></div></div><div class="rest-visual-flow"><div class="rest-visual-card is-map"><img src="__TOILET_MAP_PHOTO__" alt="トイレ位置が分かる座席表"><div class="rest-visual-caption"><span class="rest-visual-caption-icon"><i class="ph-bold ph-map-pin"></i></span><span class="rest-visual-caption-copy"><strong class="rest-visual-title">トイレの位置</strong><span class="rest-visual-text">座席表でご確認いただけます。</span></span></div></div><div class="rest-visual-card"><img src="__TOILET_ENTRANCE_PHOTO__" alt="車内トイレ入口"><div class="rest-visual-caption"><span class="rest-visual-caption-icon"><i class="ph-bold ph-door-open"></i></span><span class="rest-visual-caption-copy"><strong class="rest-visual-title">トイレ入口</strong><span class="rest-visual-text">進行方向右手中央部のカーテンを開け、階段を降ります。</span></span></div></div><div class="rest-visual-card"><img src="__TOILET_INSIDE_PHOTO__" alt="車内トイレ内部"><div class="rest-visual-caption"><span class="rest-visual-caption-icon"><i class="ph-bold ph-toilet"></i></span><span class="rest-visual-caption-copy"><strong class="rest-visual-title">トイレ内部</strong><span class="rest-visual-text">真空式トイレを採用しています。</span></span></div></div></div><div class="rest-guide-copy"><p class="rest-guide-heading">ご利用時のお願い</p><ul class="rest-guide-list"><li class="rest-guide-item"><span class="rest-guide-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 3 2.8 19h18.4L12 3Z"/><path d="M12 9v4.5"/><circle cx="12" cy="16.8" r=".8" fill="currentColor" stroke="none"/></svg></span><p class="rest-guide-text">故障の原因となりますので、備え付けの紙以外はお流しになりませんようお願いいたします。</p></li><li class="rest-guide-item"><span class="rest-guide-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="m6.2 6.2 11.6 11.6"/></svg></span><p class="rest-guide-text">トイレ内も禁煙・禁酒です。</p></li></ul></div></div></div>
                                        <div class="manner-subitem rest-break-card"><span class="manner-subitem-icon"><i class="ph-bold ph-coffee"></i></span><div><p class="manner-subitem-title">サービスエリア休憩</p><ul class="rest-info-list"><li><i class="ph-bold ph-clock"></i><span>約2〜3時間ごとに休憩します（通常2回）</span></li><li><i class="ph-bold ph-megaphone"></i><span>休憩時には乗務員が発車時刻をご案内します</span></li><li><i class="ph-bold ph-moon-stars"></i><span>休憩中は最小限の照明・ノーアナウンスでご案内します</span></li></ul><div class="rest-important-copy">休憩場所へ到着後は、車内事故防止のため、前方のカーテンが開くまでお席を離れないようお願いいたします。</div><div class="sa-board-card"><img src="__SA_TIME_BOARD_PHOTO__" alt="車両出入口横の出発時刻案内板"><div class="sa-board-caption"><i class="ph-bold ph-clock-countdown"></i><span>出発時刻は、車両出入口横の時刻案内板でご確認ください。</span></div></div></div></div>
                    ...
                    <div id="page-help" class="page-view">
                        ...
                        <div class="help-accordion-list">
                            ...
                            <div class="simple-accordion help-main-accordion">
                                ...
                                <div class="accordion-content"><div class="help-accordion-inner">
                                    <div class="help-subsection">
                                        ...
                                        <p class="help-copy">出発15分前までに集合してください。</p>
                            ...
                            <div class="simple-accordion help-main-accordion">
                                ...
                                <div class="accordion-content"><div class="help-accordion-inner">
                                    <div class="emergency-exit-section">
                                        <div class="emergency-exit-heading"><i class="ph-bold ph-map-pin"></i><span>非常口の位置</span></div>
                                        <div class="emergency-exit-tabs" role="tablist" aria-label="シートタイプを選択">
                                            <button type="button" class="emergency-exit-tab is-active" data-exit-map="3row" role="tab" aria-selected="true">3列シート</button>
                                            <button type="button" class="emergency-exit-tab" data-exit-map="4row" role="tab" aria-selected="false">4列シート</button>
                                        </div>
                                        <div class="emergency-exit-map-wrap">
                                            <img class="emergency-exit-map is-active" data-exit-panel="3row" src="__EMERGENCY_EXIT_MAP_3ROW__" alt="3列シート車両の非常口位置を示す座席表">
                                            <img class="emergency-exit-map" data-exit-panel="4row" src="__EMERGENCY_EXIT_MAP_4ROW__" alt="4列シート車両の非常口位置を示す座席表">
                                        </div>

                                        <div class="emergency-exit-heading emergency-exit-photo-heading"><i class="ph-bold ph-camera"></i><span>非常口の確認</span></div>
                                        <div class="emergency-exit-photo-grid">
                                            <figure class="emergency-exit-photo-card">
                                                <img src="__EMERGENCY_EXIT_OVERVIEW__" alt="バス後方右側にある非常口の全体写真">
                                                <figcaption>非常口の位置</figcaption>
                                            </figure>
                                            <figure class="emergency-exit-photo-card">
                                                <img src="__EMERGENCY_EXIT_HANDLE__" alt="非常口の赤い枠とハンドルの写真">
                                                <figcaption>非常口の操作部</figcaption>
                                            </figure>
                                        </div>

                                        <ul class="emergency-exit-guide-list">
                                            <li>バスの後方右側に非常口があります。</li>
                                            <li>赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。</li>
                                            <li class="emergency-exit-caution">非常の場合以外は、お手を触れないようお願い致します。</li>
                                        </ul>
                                    </div>
                                </div></div>
                            </div>
                            ...
                            <div class="simple-accordion help-main-accordion">
                                <div class="accordion-header" role="button" tabindex="0" aria-expanded="false">
                                    ...
                                    <span class="help-main-title">お問い合わせ・忘れ物</span>
                                    <span class="accordion-arrow">▶</span>
                                </div>
                                <div class="accordion-content"><div class="help-accordion-inner">
                                    <div class="help-contact-section">
                                        <div class="help-subheading"><i class="ph-bold ph-buildings"></i><span>お問い合わせ</span></div>
                                        ...
                                        <div class="help-notice-box help-contact-notice">
                                            <p class="help-notice-title"><i class="ph-bold ph-warning-circle"></i><span>お問い合わせの注意事項</span></p>
            ...
            <script>
                function initApp() {
                    ...
                    const pinConfig = {
                        ...
                        seat4: {
                            "pin-luggage":       { top: "2.6%", left: "4.8%", display: "flex" },
                    ...
                    function scheduleAnnouncementHtml(announcement) {
                        if (!announcement) return '';
                        return [
                            '    <div class="schedule-announcement-card">',
                            '      <div class="schedule-announcement-header" role="button" tabindex="0" aria-expanded="false">',
                            '        <div class="schedule-announcement-title"><i class="ph-bold ph-megaphone"></i><span>' + escapeScheduleHtml(announcement.type) + '</span></div>',
                            '        <span class="schedule-announcement-arrow">▶</span>',
                            '      </div>',
                            '      <div class="schedule-announcement-content"><div class="schedule-announcement-inner">',
                            '        <div class="schedule-announcement-text">' + escapeScheduleHtml(announcement.text) + '</div>',
                            '      </div></div>',
                            '    </div>'
                        ].join('');
                    }

                    function createScheduleTimelineNode(event, announcements) {
                        const announcement = event.announcementKey ? announcements[event.announcementKey] : null;
                        return [
                            '  <div class="image-timeline-node node-' + escapeScheduleHtml(event.tone) + '">',
                            '    <div class="node-header"><span class="node-event-icon"><i class="ph-bold ' + escapeScheduleHtml(event.icon) + '"></i></span><span class="node-time">' + escapeScheduleHtml(event.time) + '</span><span class="node-title">' + escapeScheduleHtml(event.title) + '</span></div>',
                            '    <div class="node-desc">' + escapeScheduleHtml(event.desc) + '</div>',
                            scheduleAnnouncementHtml(announcement),
                            '  </div>'
                        ].join('');
                    }

                    function createSchedule(title, events, announcements) {
                        return {
                            title: title,
                            events: events,
                            ...
                            html: [
                                ...
                                events.map(event => createScheduleTimelineNode(event, announcements || {})).join(''),
                                '</div>'
                            ].join('')
                        };
                    }
                    ...
                    const schedules = {
                        bus_nt1001: createSchedule("ナイトライナー NT1001便（東京⇒大阪・京都）4列スタンダード", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "21:20",
                                    "title": "乗車地①",
                                    "desc": "バスタ新宿4F＜新宿駅南口高速バスターミナル＞"
                                },
                                {
                                    "tone": "red",
                                    "icon": "ph-bus",
                                ...
                                },
                                {
                                    ...
                                    "time": "22:05",
                                    "title": "乗車地②",
                                    "desc": "バスターミナル東京八重洲 地下1F＜東京駅八重洲北口 トフロムヤエスタワーB1F＞"
                                },
                                {
                                    "tone": "red",
                                    "icon": "ph-bus",
                                    "time": "22:15",
                                    "title": "出発",
                                    "desc": "バスターミナル東京八重洲を出発",
                                    "announcementKey": "22:15"
                                },
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                ...
                                },
                                {
                                    ...
                                    "time": "06:30",
                                    "title": "降車地②",
                                    "desc": "プラザモータープール＜大阪梅田＞",
                                    "announcementKey": "06:30"
                                },
                                {
                                    "tone": "red",
                                    "icon": "ph-bus",
                                ...
                                },
                                {
                                    ...
                                    "time": "07:00",
                                    "title": "降車地③",
                                    "desc": "天王寺公園バス駐車場（てんしば＋プラス前）",
                                    "announcementKey": "07:00"
                                }
                            ], {
                                "21:30": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「天王寺公園東バス駐車場」行きです。 この後「バスターミナル東京八重洲地下A」「八王子駅南口バスターミナル」を経由しまして関西方面へ進めて参ります。 車内は全席指定、禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますので、シートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 その他、詳しいご案内は最終経由地「八王子駅南口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "22:15": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「天王寺公園東バス駐車場」行きです。 この後「八王子駅南口バスターミナル」を経由しまして関西方面へ進めて参ります。 車内は全席指定、禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますのでシートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 その他、詳しいご案内は最終経由地「八王子駅南口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "23:30": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「天王寺公園東バス駐車場行き」です。 各地の到着予定時刻は「京都駅八条口」・明朝5時40分、「プラザモータープール＜大阪梅田＞」・6時30分、終点「天王寺公園東バス駐車場」・7時ちょうどを予定しております. なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を2回ほど予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:40": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「京都駅八条口」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                },
                                "06:30": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「プラザモータープール＜大阪梅田＞」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                },
                                "07:00": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「天王寺公園東バス駐車場」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                }
                            }),
                        bus_nt002: createSchedule("ナイトライナー NT002便（大阪・京都⇒高尾・新宿・東京）4列スタンダード", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "21:35",
                                ...
                                },
                                {
                                    ...
                                    "time": "06:48",
                                    "title": "降車地②",
                                    "desc": "バスタ新宿3F＜新宿駅南口高速バスターミナル＞",
                                    "announcementKey": "06:48"
                                },
                                {
                                    "tone": "red",
                                    "icon": "ph-bus",
                                ...
                                },
                                {
                                    ...
                                    "title": "降車地③",
                                    "desc": "バスターミナル東京八重洲 地下1F＜東京駅八重洲北口 トフロムヤエスタワーB1F＞",
                                    "announcementKey": "07:30"
                                }
                            ], {
                                "21:45": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「バスターミナル東京八重洲地下A」行きです。 この後「プラザモータープール＜大阪梅田＞ 」「京都駅八条口 」を経由しまして関東方面へ進めて参ります。 車内は全席指定、禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますので、シートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 その他、詳しいご案内は最終経由地「京都駅八条口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "22:30": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「バスターミナル東京八重洲地下A」行きです。 この後「京都駅八条口」を経由しまして関東方面へ進めて参ります。 車内は全席指定、禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますのでシートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 その他、詳しいご案内は最終経由地「京都駅八条口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "23:30": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「バスターミナル東京八重洲地下A」行きです。 各地の到着予定時刻は、「高尾駅南口」・明朝5時40分、「バスタ新宿3F」・6時48分、終点「バスターミナル東京八重洲地下A」・7時30分を予定しております. なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を2回ほど予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:40": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「高尾駅南口」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                },
                                "06:48": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「バスタ新宿3F」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                },
                                "07:30": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「バスターミナル東京八重洲地下A」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                }
                            }),
                        bus_nt1003: createSchedule("ナイトライナー NT1003便（東京⇒奈良・大阪・京都）3列独立（トイレ有）", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "21:20",
                                ...
                                },
                                {
                                    ...
                                    "title": "降車地③",
                                    "desc": "プラザモータープール＜大阪梅田＞",
                                    "announcementKey": "07:15"
                                }
                            ], {
                                "21:30": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「プラザモータープール＜大阪梅田＞」行きです。 この後「バスターミナル東京八重洲地下A」「横浜シティ・エア・ターミナル」を経由しまして関西方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますので、シートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「横浜シティ・エア・ターミナル」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "22:15": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「プラザモータープール＜大阪梅田＞」行きです。 この後「横浜シティ・エア・ターミナル」を経由しまして関西方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますのでシートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「横浜シティ・エア・ターミナル」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "23:10": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「プラザモータープール＜大阪梅田＞」行きです。 各地の到着予定時刻は「京都駅八条口」・明朝5時20分、「奈良駅東口」・6時20分、終点「プラザモータープール＜大阪梅田＞」・7時15分を予定しております. なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。故障の原因となりますので備え付けの紙以外はお流しになりません様、お願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を2回ほど予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:20": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「京都駅八条口」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                ...
                                "06:20": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「奈良駅東口」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                },
                                "07:15": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「プラザモータープール＜大阪梅田＞」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                        ...
                        bus_nt004: createSchedule("ナイトライナー NT004便（大阪・奈良・京都⇒横浜・新宿・東京）3列独立（トイレ有）", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "21:40",
                                ...
                                },
                                {
                                    ...
                                    "title": "降車地③",
                                    "desc": "バスターミナル東京八重洲 地下1F＜東京駅八重洲北口 トフロムヤエスタワーB1F＞",
                                    "announcementKey": "07:06"
                                }
                            ], {
                                "21:50": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「バスターミナル東京八重洲地下A」行きです。 この後「奈良駅東口 」「京都駅八条口 」を経由しまして関東方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますので、シートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「京都駅八条口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "22:45": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「バスターミナル東京八重洲地下A」行きです。 この後「京都駅八条口」を経由しまして関東方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますのでシートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「京都駅八条口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "23:45": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「バスターミナル東京八重洲地下A」行きです。 各地の到着予定時刻は、「横浜駅東口スカイビル＜YCAT＞前」・明朝5時40分、「バスタ新宿3F」・6時27分、終点「バスターミナル東京八重洲地下A」・7時06分を予定しております. なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。故障の原因となりますので備え付けの紙以外はお流しになりません様、お願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を2回ほど予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:40": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「横浜駅東口スカイビル＜YCAT＞前」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                },
                                "06:27": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「バスタ新宿3F」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                        ...
                        bus_nt1005: createSchedule("ナイトライナー NT1005便（大宮・新宿・八王子⇒京都・大阪・天王寺）3列独立（トイレ有）", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "21:30",
                                    "title": "乗車地①",
                                    "desc": "大宮駅西口＜ソニックシティビル前＞"
                                },
                                {
                                    "tone": "red",
                                    "icon": "ph-bus",
                                ...
                                },
                                {
                                    ...
                                    "time": "07:05",
                                    "title": "降車地②",
                                    "desc": "OCAT 難波バスターミナル2F 1・2・3・9・10番のりば",
                                    "announcementKey": "07:05"
                                },
                                {
                                    "tone": "red",
                                    "icon": "ph-bus",
                                ...
                                },
                                {
                                    ...
                                    "title": "降車地③",
                                    "desc": "天王寺公園バス駐車場（てんしば＋プラス前）",
                                    "announcementKey": "07:40"
                                }
                            ], {
                                "21:40": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「天王寺公園東バス駐車場」行きです。 この後「バスタ新宿4F」「八王子駅南口バスターミナル」を経由しまして関西方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますので、シートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「八王子駅南口バスターミナル」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "22:40": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー　「天王寺公園東バス駐車場」行きです。 この後「八王子駅南口バスターミナル」を経由しまして関西方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますのでシートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「八王子駅南口バスターミナル」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "23:45": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「天王寺公園東バス駐車場」行きです。 各地の到着予定時刻は「京都駅八条口」・明朝6時ちょうど、「OCAT 難波バスターミナル」・7時5分、終点「天王寺公園東バス駐車場」・7時40分を予定しております. なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。故障の原因となりますので備え付けの紙以外はお流しになりません様、お願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を2回ほど予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "06:00": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「京都駅八条口」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                ...
                                "07:05": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「OCAT 難波バスターミナル」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                },
                                "07:40": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「天王寺公園東バス駐車場」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                        ...
                        bus_nt006: createSchedule("ナイトライナー NT006便（天王寺・大阪・京都⇒高尾・新宿・大宮）3列独立（トイレ有）", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "21:50",
                                ...
                                },
                                {
                                    ...
                                    "title": "降車地③",
                                    "desc": "大宮駅西口＜ソニックシティビル前＞",
                                    "announcementKey": "07:50"
                                }
                            ], {
                                "22:00": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「大宮駅西口＜ソニックシティビル前＞」行きです。 この後「OCAT　難波バスターミナル 」「京都駅八条口 」を経由しまして関東方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますので、シートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「京都駅八条口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                },
                                "22:25": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「大宮駅西口＜ソニックシティビル前＞」行きです。 この後「京都駅八条口」を経由しまして関東方面へ進めて参ります。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。 また、法令により全席シートベルトの着用が義務付けられておりますのでシートベルトの着用をお願い致します。 各お座席にミネラルウォーターをご用意しておりますのでどうぞご自由にお召し上がり下さい。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。 その他、詳しいご案内は最終経由地「京都駅八条口」を出発した後にご案内させていただきます。 それではしばらくの間、ごゆっくりとお過ごし下さい。"
                                ...
                                "23:30": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「大宮駅西口＜ソニックシティビル前＞」行きです。 各地の到着予定時刻は、「高尾駅南口」・明朝5時40分、「バスタ新宿3F」・6時48分、終点「大宮駅西口＜ソニックシティビル前＞」・7時50分を予定しております. なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、化粧室を含め禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 化粧室は進行方向右手中央部のカーテンを開け、階段を降りた所にございます。故障の原因となりますので備え付けの紙以外はお流しになりません様、お願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を2回ほど予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:40": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「高尾駅南口」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                ...
                                "07:50": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「大宮駅西口＜ソニックシティビル前＞」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。引き換え札をお手元にご用意の上、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                }
                            }),
                        bus_nt011: createSchedule("ナイトライナー NT011便（新宿⇒仙台）4列スタンダード", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "22:55",
                                ...
                                },
                                {
                                    ...
                                    "time": "05:00",
                                    "title": "降車地",
                                    "desc": "ヨドバシ仙台 第一ビルバスターミナル",
                                    "announcementKey": "05:00"
                                }
                            ], {
                                "23:05": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「ヨドバシ仙台　第一ビルバスターミナル」行きです。 「ヨドバシ仙台　第一ビルバスターミナル」到着時刻は明朝5時ちょうどを予定しております。 なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を3回ほど予定しております。道路状況により、前後する可能性もございますが、1時00分前後、2時30分前後、4時ちょうど前後を予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:00": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「ヨドバシ仙台　第一ビルバスターミナル」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。お取り忘れのございませんよう、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                                }
                            }),
                        bus_nt012: createSchedule("ナイトライナー NT012便（仙台⇒新宿）4列スタンダード", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "23:10",
                                ...
                                },
                                {
                                    ...
                                    "time": "23:20",
                                    "title": "出発",
                                    "desc": "ヨドバシ仙台 第一ビルバスターミナルを出発",
                                    "announcementKey": "23:20"
                                },
                                {
                                    "tone": "red",
                                    "icon": "ph-moon-stars",
                                ...
                                },
                                {
                                    ...
                                    "title": "降車地",
                                    "desc": "バスタ新宿3F＜新宿駅南口高速バスターミナル＞",
                                    "announcementKey": "05:30"
                                }
                            ], {
                                "23:20": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「バスタ新宿3F」行きです。 「バスタ新宿3F」到着時刻は明朝5時30分を予定しております。 なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を3回ほど予定しております。道路状況により、前後する可能性もございますが、1時00分前後、2時30分前後、4時ちょうど前後を予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:30": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「バスタ新宿3F」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。お取り忘れのございませんよう、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                        ...
                        bus_nt013: createSchedule("ナイトライナー NT013便（東京⇒仙台）4列スタンダード", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "24:05（翌00:05）",
                                ...
                                "24:15": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「ヨドバシ仙台　第一ビルバスターミナル」行きです。 「ヨドバシ仙台　第一ビルバスターミナル」到着時刻は明朝6時ちょうどを予定しております。 なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を3回ほど予定しております。道路状況により、前後する可能性もございますが、1時00分前後、2時30分前後、4時ちょうど前後を予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "06:00": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「ヨドバシ仙台　第一ビルバスターミナル」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。お取り忘れのございませんよう、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                        ...
                        bus_nt014: createSchedule("ナイトライナー NT014便（仙台⇒東京）4列スタンダード", [
                                {
                                    "tone": "blue",
                                    "icon": "ph-clipboard-text",
                                    "time": "23:40",
                                ...
                                },
                                {
                                    ...
                                    "title": "降車地",
                                    "desc": "バスターミナル東京八重洲 地下1F＜東京駅八重洲北口 トフロムヤエスタワーB1F＞",
                                    "announcementKey": "05:54"
                                }
                            ], {
                                "23:50": {
                                    "type": "出発アナウンス",
                                    "text": "本日は東京富士交通をご利用いただきまして誠にありがとうございます。 このバスはナイトライナー「バスターミナル東京八重洲地下A」行きです。 「バスターミナル東京八重洲地下A」到着時刻は明朝5時54分を予定しております。 なお、途中の道路状況や天候などにより、一般道路への迂回、またはサービスエリアなどで待機する場合がございます。その際は、到着予定時刻より大幅に遅れる場合もございますので、ご了承ください。 安全運転には十分心がけておりますが、事故防止の為、急停車や急ハンドルを切る場合がございます。シートベルトの着用をお願い致します。 車内は全席指定、禁煙、禁酒となっております。携帯電話は電源を切るかマナーモードに設定の上、通話はご遠慮いただきますよう、ご理解とご協力をお願い致します。 座席はリクライニングシートです。座席横のレバーを操作すると座席は後部に倒れます。 ご利用の際は後ろのお客さまにお声掛けください。 またドリンクホルダーにはミネラルウォーターをご用意しておりますのでご自由にお召し上がり下さい。 お手持ちの荷物は通路には置かず、荷物棚か足元に納めていただきますようお願い致します。 このバスの後方右側に非常口がございます。赤い枠をはずし、ハンドルを矢印の方向に回すと扉は外に開きます。 非常の場合以外はお手を触れないようお願い致します。 このあとバスは高速道路に入ります。途中、サービスエリアで15分前後の休憩を3回ほど予定しております。道路状況により、前後する可能性もございますが、1時00分前後、2時30分前後、4時ちょうど前後を予定しております。 各休憩場所到着時のアナウンスは差し控えさせていただいております。到着時は最低限の照明を点灯し、前方のカーテンが開くまでは車内事故防止の為、お席を離れない様、お願い致します。 出発時刻は車両出入口横の時刻案内板にてご案内しております。 出発の際、乗務員がお客様の人数確認をいたしますが、原則、定刻に出発致しますのでご案内した時間までにお席にお戻り下さいますようお願い致します。 その他、何かございましたら乗務員までお申しつけ下さい。 このあと5分程で車内は消灯いたします。 消灯後の私語などは他のお客様にご迷惑となりますのでご遠慮下さい。 また窓側のカーテンは夜間走行の為、お閉めいただきますようお願い致します。 車内のお願い事ばかりで大変恐縮ではございますが、狭い車内、ごゆっくりとおくつろぎいただけますよう皆様のご理解とご協力をお願いいたします。 それでは消灯まで今しばらくお待ち下さい。"
                                },
                                "05:54": {
                                    "type": "到着アナウンス",
                                    "text": "おはようございます。 大変、長らくのご乗車お疲れ様でございました。 このバスは間もなく「バスターミナル東京八重洲地下A」に到着いたします。 車内に落し物、お忘れ物ございませぬ様、ご注意下さい。 車内で出たゴミなどは、お降りの際に出入口にございますゴミ袋へお捨て下さい。 車内美化にご協力をお願い致します。 リクライニングシートは元の位置にお戻し下さい。 また、お預かりしましたお荷物は乗務員がお渡しいたします。お取り忘れのございませんよう、トランクまでお越し下さい。 車内事故防止の為、車両停止後、車内前方のカーテンが開くまでお席でお待ち下さい。 本日は東京富士交通ナイトライナーにご乗車いただきまして誠にありがとうございました。 またのご乗車を乗務員一同心よりお待ちしております。"
                    ...
                    const seatFeatureData = {
                        ...
                        tableLegrest: {
                            title: "簡易テーブル・レッグレストバー",
                            desc: "簡易テーブルとレッグレストバーの位置です。",
                            detail: '<div class="seat-detail-guide"><div class="seat-equipment-block"><div class="seat-equipment-title"><i class="ph-bold ph-tray"></i><span>簡易テーブル</span></div><div class="seat-equipment-stack"><div class="seat-equipment-step-card"><img class="seat-equipment-step-photo" src="__SIDE_TABLE_DEPLOYED_PHOTO__" alt="簡易テーブルがアームレスト内に収納されている状態の写真"><strong>収納状態</strong><span>簡易テーブルはアームレスト内に収納されています。</span></div><div class="seat-equipment-step-card"><img class="seat-equipment-step-photo" src="__SIDE_TABLE_UNLOCK_PHOTO__" alt="アームレスト上部を開けて簡易テーブルを取り出す写真"><strong>取り出し方</strong><span>アームレスト上部を開け、中から簡易テーブルを手動で引き出してください。</span></div></div></div><div class="seat-equipment-block"><div class="seat-equipment-title"><i class="ph-bold ph-armchair"></i><span>レッグレストバー</span></div><div class="seat-equipment-photo-card"><img class="seat-equipment-photo seat-equipment-photo-contain" src="__LEGREST_OPERATION_PHOTO__" alt="アームレスト先端のボタンとレッグレストが分かる写真"><span>アームレスト先端のボタンを押すと、レッグレストが動きます。</span></div></div></div>'
                        },
                        privacy: {
                            title: "カーテン・仕切り",
                        ...
                        usb: {
                            ...
                            detail: '<div class="seat-detail-guide"><div class="seat-equipment-block"><div class="seat-equipment-title"><i class="ph-bold ph-usb"></i><span>USBポート</span></div><div class="seat-equipment-photo-card"><img class="seat-equipment-photo seat-equipment-photo-contain" src="__SEAT3_USB_PHOTO__" alt="3列独立シートのUSBポート位置"><span>アームレスト下部にあるUSBポートです。</span></div><p class="seat-equipment-text">USB対応機器の充電にご利用いただけます。充電ケーブルはお客様ご自身でご用意ください。</p></div></div>'
                        },
                        footcomfort: {
                            title: "フットレスト",
                    ...
                    seat4FeatureData.overhead = {
                        title: "読書灯・個別空調",
                        desc: "各座席の上部（窓側上部）に設置されています。",
                        detail: `<div class="seat-detail-guide">
                            <img class="seat-detail-photo seat-detail-photo-tall" src="__SEAT4_OVERHEAD_PHOTO__" alt="4列シートの読書灯・個別空調">
                            <div class="seat-equipment-block">
                                <div class="seat-equipment-title"><i class="ph-bold ph-map-pin"></i><span>設置場所</span></div>
                                <p class="seat-equipment-text">読書灯・個別空調は、各座席の上部（窓側上部）に設置されています。</p>
                            </div>
                            <div class="seat-equipment-block">
                                <div class="seat-equipment-title"><i class="ph-bold ph-lightbulb"></i><span>読書灯</span></div>
                                <p class="seat-equipment-text">オレンジ色のボタンを押すと点灯します。消灯後は、周囲のお客様へのご配慮をお願いいたします。</p>
                    ...
                    const stationGroups = {
                        kanto: [
                            "バスタ新宿4F＜新宿駅南口高速バスターミナル＞",
                            "八王子駅南口バスターミナル3番・4番バス停",
                            "高尾駅南口",
                            "バスターミナル東京八重洲地下A＜東京駅八重洲北口 トフロムヤエスタワー B1F＞",
                            "大宮駅西口＜ソニックシティビル前＞",
                            "横浜シティ・エア・ターミナル＜YCAT第1ロビー＞"
                        ],
                        kansai: [
                            "ＯＣＡＴ 難波バスターミナル２Ｆ １・２・３・９・１０番のりば",
                            "プラザモータープール＜大阪梅田＞",
                            "天王寺公園東バス駐車場（てんしば＋プラス前）",
                            "京都駅八条口 観光バス乗降場",
                            "奈良駅東口＜4番バス停＞"
                        ],
                        tohoku: [
                    ...
                    const stationMeta = {
                        "バスタ新宿4F＜新宿駅南口高速バスターミナル＞": "JR新宿駅新南口直結",
                        "八王子駅南口バスターミナル3番・4番バス停": "JR八王子駅 南口",
                        "高尾駅南口": "JR高尾駅 南口",
                        "バスターミナル東京八重洲地下A＜東京駅八重洲北口 トフロムヤエスタワー B1F＞": "地下A",
                        "大宮駅西口＜ソニックシティビル前＞": "JR大宮駅 西口",
                        "横浜シティ・エア・ターミナル＜YCAT第1ロビー＞": "YCAT第1ロビー",
                        "ＯＣＡＴ 難波バスターミナル２Ｆ １・２・３・９・１０番のりば": "JR難波駅直結 OCAT",
                        "プラザモータープール＜大阪梅田＞": "大阪・梅田",
                        "天王寺公園東バス駐車場（てんしば＋プラス前）": "てんしば＋プラス前",
                        "京都駅八条口 観光バス乗降場": "京都駅 八条口",
                        "奈良駅東口＜4番バス停＞": "4番バス停",
                        "ヨドバシ仙台　第一ビルバスターミナル": "仙台駅東口"
                    };

                    const stationDetails = {
                        "バスタ新宿4F＜新宿駅南口高速バスターミナル＞": [
                            '<p class="station-guide-text">東京都渋谷区千駄ヶ谷5-24-55</p>',
                            '<p class="station-guide-text">JR新宿駅新南口直結（徒歩約2分）</p>',
                            '<p class="station-guide-text">※新宿バスタ待合所にて出発便の番線案内がございますので、お間違えないようにご注意ください。出発に余裕を持つため、15分前に集合して頂くようお願い致します。</p>'
                        ].join(''),
                        "八王子駅南口バスターミナル3番・4番バス停": '',
                        "高尾駅南口": '',
                        "バスターミナル東京八重洲地下A＜東京駅八重洲北口 トフロムヤエスタワー B1F＞": [
                            '<p class="station-guide-text">TOFROM YAESU(トフロム ヤエス)地下B1F<br>バスターミナル東京八重洲　地下A</p>',
                            '<div class="station-guide-important"><strong>【重要】「地下A」「地下B」のお間違いにご注意ください</strong>バスターミナル東京八重洲には、「地下A」と「地下B」の2つのエリアがあり、それぞれ離れた別の乗り場です。<br>ご利用の際は、乗り場が「地下A」「地下B」のどちらであるか、必ず事前にご確認ください。<br>特に23時以降は地下街の通路が一部閉鎖され、地下街を通って地下A・地下B間を移動できなくなります。時間に余裕をもってお越しください。<br>（バスは決められた時間に出発します。乗り遅れの返金はございません）</div>',
                            '<a class="station-guide-link" href="https://bt-tokyoyaesu.com/20260323-2/" target="_blank" rel="noopener noreferrer">▶ 「地下A」「地下B」の乗り場間違いに関するご案内</a>',
                            '<a class="station-guide-link" href="https://bt-tokyoyaesu.com/accessmap-a/#accessmap-tab" target="_blank" rel="noopener noreferrer">詳細なアクセスについてはこちらのページをご参照ください。</a>'
                        ].join(''),
                        "ＯＣＡＴ 難波バスターミナル２Ｆ １・２・３・９・１０番のりば": [
                            '<p class="station-guide-text">9番のりば前のカウンター付近にお越し下さい。(出発モニターを設置しております。)</p>',
                            '<p class="station-guide-text">バスは『1・2・3・9・10番のりば』のいづれかに配車します。<br>準備ができ次第、出発モニターにて表示とアナウンスを致します。<br>事前の受付等は必要ありません。乗車時にバス運転手にお名前をお伝えください。</p>',
                            '<p class="station-guide-text">※乗り遅れ等があっても一切の責任は負いません。時間には余裕をもってお越し下さい。<br>※ご不明な点は、オレンジ色のベストの係員にお尋ね下さい。</p>'
                        ].join(''),
                        "プラザモータープール＜大阪梅田＞": '',
                        "天王寺公園東バス駐車場（てんしば＋プラス前）": '',
                        "京都駅八条口 観光バス乗降場": '',
                        "大宮駅西口＜ソニックシティビル前＞": '',
                        "横浜シティ・エア・ターミナル＜YCAT第1ロビー＞": '<p class="station-guide-text">横浜駅からYCATへはこちらをご覧ください。</p><a class="station-guide-link" href="https://www.ycat.co.jp/access/station2ycat/" target="_blank" rel="noopener noreferrer">YCATへのアクセス案内</a>',
                        "奈良駅東口＜4番バス停＞": '',
                        "ヨドバシ仙台　第一ビルバスターミナル": ''
                    };
                    ...
                    function bindModalEvents() {
                        ...
                        document.querySelectorAll('.schedule-announcement-header').forEach(header => {
                            header.addEventListener('click', function() {
                                const card = this.closest('.schedule-announcement-card');
                                if (!card) return;
                    ...
                    function showStationMenuModal() {
                        ...
                        html += '<div class="station-menu-group-title group-tohoku"><i class="ph-bold ph-map-trifold"></i>東北エリア</div>';
                        stationGroups.tohoku.forEach(name => {
                            html += stationMenuCard(name);
                        });
                        html += '</div>';
                    ...
                    function showStationDetail(name) {
                        ...
                        const html = [
                            ...
                            '  <div class="station-guide-card">',
                            '      <div class="station-guide-label">【乗車地名】</div>',
                            `      <div class="station-guide-name">${name}</div>`,
                            detail,
                            '  </div>',
                            '</div>'
                        ].join('');
                    ...
                    const amenityDetailData = {
                        blanket: {
                            ...
                            html: '大判ブランケットをお一人様1枚、座席にご用意しております。<br>ご利用ごとに洗濯・クリーニングを行い、清潔なブランケットをご提供しております。<span class="amenity-detail-highlight">3列独立シートをご利用のお客様は、乗務員へお声がけいただくと、もう1枚追加で貸し出し可能です。</span>'
                        },
                        water: {
                            title: 'ミネラルウォーター',
                            icon: 'ph-drop',
                            photo: '__AMENITY_WATER_PHOTO__',
                            html: 'ペットボトルのミネラルウォーターを、お一人様1本ご用意しております。'
                        },
                        cushion: {
                            title: '腰あてクッション',
                            icon: 'ph-sidebar-simple',
                            photo: '__AMENITY_CUSHION_PHOTO__',
                            html: '長時間でも快適にお過ごしいただけるよう、腰あてクッションを全席にご用意しております。'
                        },
                        mask: {
                            title: 'フェイスマスク',
                            icon: 'ph-smiley-blank',
                            photo: '__AMENITY_MASK_PHOTO__',
                            html: 'オルフェスのフェイスマスクを全席にご用意しております。'
                        }
                    };
                    ...
                    document.querySelectorAll('.amenity-select-card').forEach(card => {
                        card.addEventListener('click', function(e) {
                            ...
                            const modalHtml = [
                                ...
                                '  <div class="amenity-modal-photo-wrap"><img class="amenity-modal-photo" src="' + data.photo + '" alt="' + data.title + 'の写真"></div>',
                                '  <div class="amenity-modal-description">' + data.html + '</div>',
                                '</div>'
                            ].join('');
                            openModal(data.title, modalHtml);
                        });
                    });
                    ...
                    const emergencyExitTabs = document.querySelectorAll('.emergency-exit-tab');
                    const emergencyExitMaps = document.querySelectorAll('.emergency-exit-map');
                    emergencyExitTabs.forEach(function(tab) {
                        tab.addEventListener('click', function() {
                            const target = tab.getAttribute('data-exit-map');
                            emergencyExitTabs.forEach(function(item) {
                                const active = item === tab;
                                item.classList.toggle('is-active', active);
                                item.setAttribute('aria-selected', active ? 'true' : 'false');
                            });
                            emergencyExitMaps.forEach(function(map) {
                                map.classList.toggle('is-active', map.getAttribute('data-exit-panel') === target);
                            });
                        });
                    });
    ...
    html = (
        ...
        .replace("__SEAT4_OVERHEAD_PHOTO__", seat4_overhead_photo)
        ...
        .replace("__SEAT3_USB_PHOTO__", seat3_usb_photo)
        .replace("__FOOTREST_PHOTO__", footrest_photo)
        .replace("__AMENITY_BLANKET_PHOTO__", amenity_blanket_photo)
        .replace("__AMENITY_WATER_PHOTO__", amenity_water_photo)
        .replace("__AMENITY_CUSHION_PHOTO__", amenity_cushion_photo)
        .replace("__AMENITY_MASK_PHOTO__", amenity_mask_photo)
        .replace("__TOILET_MAP_PHOTO__", toilet_map_photo)
        .replace("__TOILET_ENTRANCE_PHOTO__", toilet_entrance_photo)
        .replace("__TOILET_INSIDE_PHOTO__", toilet_inside_photo)
        .replace("__SA_TIME_BOARD_PHOTO__", sa_time_board_photo)
        .replace("__EMERGENCY_EXIT_MAP_3ROW__", emergency_exit_map_3row)
        .replace("__EMERGENCY_EXIT_MAP_4ROW__", emergency_exit_map_4row)
        .replace("__EMERGENCY_EXIT_OVERVIEW__", emergency_exit_overview)
        .replace("__EMERGENCY_EXIT_HANDLE__", emergency_exit_handle)
    )
